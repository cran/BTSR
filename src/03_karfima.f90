...
contains
  ...
  subroutine loglik_karfima(model, npar, par, sll, U)
    !------------------------------------------------------------------
    !
    !   Log-likelihood: KARFIMA model
    ...
    call loglik_generic(llk_kuma, dllk_kuma, model, npar, par, sll, U)
    if(sll < -Huge(1.d0)) sll = -Huge(1.d0)
    if(sll > Huge(1.d0)) sll = Huge(1.d0)
    return
  end subroutine loglik_karfima

  subroutine loglik_karfima_nelder(model, npar, par, sll)
    !------------------------------------------------------------------
    !
    ...
    model%sco = 0
    call loglik_karfima(model, npar, par_aux, sll, U)
    return
  end subroutine loglik_karfima_nelder

end module karfima

subroutine karfimaR(n, y, gy, ystart, nreg, xreg, xstart, mu, eta, error, escale,&
  ...
  !     nu = value of nu (if fixed). Dummy value when nu is non-fixed.
  ...
  real(dp) :: par(npar), alpha, d, pdist(4), beta(max(1,fixB))
  ...
  !--------------------------------------------------------------------------------   
  call get_model(model, n, y, pdist(3), pdist(4), ystart, gy, nreg, xreg, xstart, link,  &
  ...
  model%argsD%lower = pdist(3)
  model%argsD%upper = pdist(4)

  call final_model(model, npar, par, fixnu, n, mu, eta, error, nnew, nreg, &
       xnew, ynew, inf, sll, sco, U, info, K, extra, Drho, T, E, h, &
       llk_kuma, dllk_kuma, Ed2llk_kuma)

  return
end subroutine karfimaR

subroutine optimnelderkarfimaR(npar, par, nbd, lower, upper, n, y, gy, ystart, nreg, &
  ...
  ! Nelder-Mead
  !-------------------------------------------------------------------------------
  call optim(loglik_karfima_nelder, model, npar, par, nbd, lower, upper, sll, iprint,&
  ...
  return      
end subroutine optimnelderkarfimaR

subroutine optimlbfgsbkarfimaR(npar, par, nbd, lower, upper, n, y, gy, ystart, nreg, &
  ...
  ! L-BFGS-B
  !----------------------------------------------------------------------------------
  call optim(loglik_karfima, model, npar, par, nbd, lower, upper, sll, U, iprint, &
...
subroutine simkarfimaR(n, burn, pdist, alpha, nreg, beta, p, phi, &
  ...
  call sim_model(rkuma, n, burn, 4, pdist, alpha, nreg, beta, p, phi, q, theta, &
       d, linkg, xreg, xregar, yt, pdist(3), pdist(4), ystart, xstart, mu, eta, &
       error, escale, ns, seed, rngtype, inf, rev, argsL)

  return
end subroutine simkarfimaR
