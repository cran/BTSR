...
contains
  ...
  subroutine loglik_barfima(model, npar, par, sll, U)
    !------------------------------------------------------------------
    !
    ...
    call loglik_generic(llk_beta, dllk_beta, model, npar, par, sll, U)
    if(sll < -Huge(1.d0)) sll = -Huge(1.d0)
    if(sll > Huge(1.d0)) sll = Huge(1.d0)
    return
  end subroutine loglik_barfima

  subroutine loglik_barfima_nelder(model, npar, par, sll)
    !------------------------------------------------------------------
    !
    ...
    model%sco = 0
    call loglik_barfima(model, npar, par_aux, sll, U)
    return
  end subroutine loglik_barfima_nelder

end module barfima

subroutine barfimaR(n, y, gy, ystart, nreg, xreg, xstart, mu, eta, error, escale,&
     nnew, xnew, ynew, link, npar, par, fixa, alpha, fixB, flagsb, beta, p, fixar, &
     flagsar, phi, xregar,  q, fixma, flagsma, theta, fixd, d, fixnu, nu, inf, m, &
  ...
  !--------------------------------------------------------------------------------   
  call get_model(model, n, y,  0.d0, 1.d0, ystart, gy, nreg, xreg, xstart, link, escale, &
       0, npar, par, fixa, alpha, fixB, flagsb, beta, p, fixar, flagsar, phi, xregar,&
  ...
  call final_model(model, npar, par, fixnu, n, mu, eta, error, nnew, nreg, &
       ...
       llk_beta, dllk_beta, Ed2llk_beta)

  return
end subroutine barfimaR

subroutine optimnelderbarfimaR(npar, par, nbd, lower, upper, n, y, gy,ystart, nreg, xreg,&
  ...
  ! Nelder-Mead
  !-------------------------------------------------------------------------------
  call optim(loglik_barfima_nelder, model, npar, par, nbd, lower, upper, sll, iprint,&
  ...
  return      
end subroutine optimnelderbarfimaR

subroutine optimlbfgsbbarfimaR(npar, par, nbd, lower, upper, n, y, gy, ystart, nreg, xreg,&
     xstart, mu, eta, error, escale, nnew, xnew, ynew, link, fixa, alpha, fixB, flagsb, beta, &
     p, fixar, flagsar, phi, xregar, q, fixma, flagsma, theta, fixd, d, fixnu, nu, inf, m,&
  ...
  ! L-BFGS-B
  !----------------------------------------------------------------------------------
  call optim(loglik_barfima, model, npar, par, nbd, lower, upper,&
  ...
  return
end subroutine optimlbfgsbbarfimaR

subroutine simbarfimaR(n, burn, nu, alpha, nreg, beta, p, phi, q, theta, d, &
     linkg, xreg, xregar, yt, ystart, xstart, mu, eta, error, escale, &
     ns, seed, rngtype, inf, rev)
  use barfima
  !#---------------------------------------------------------------------
  !#
  !#  Simulating a BARFIMA model
  !#
  !#---------------------------------------------------------------------
  implicit none
  ...
  call sim_model(rbeta, n, burn, 1, (/nu/), alpha, nreg, beta, p, phi, q, theta, d, &
       linkg, xreg, xregar, yt, 0.d0, 1.d0,  ystart, xstart, mu, eta, error, &
       escale, ns, seed, rngtype, inf, rev, argsL)
  return
end subroutine simbarfimaR
